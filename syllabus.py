...
class TimeTable:
    def __init__(self):
        ...
        self.rooms = dict()
        self.roomcounter = 0
        self.table = np.zeros((0, 0), dtype=int)

    def add(self, time, room, class_id):
        if time not in self.time:
            self.table = np.concatenate((
                self.table,
                np.zeros((1, self.roomcounter), dtype=int)
            ), 0)
        ...
        if room not in self.rooms:
            self.table = np.concatenate((
                self.table,
                np.zeros((self.timecounter, 1), dtype=int)
            ), 1)
            self.rooms[room] = self.roomcounter
            self.roomcounter += 1
        self.table[self.time[time], self.rooms[room]] = class_id
...
def database2table(table):
    with SessionContext() as session:
        for c in session.query(ClassInfo).all():
            table.add(time2str(c.weekday, c.period), room2str(c.building, c.room), c.id)
...
def insert_class_info(table):
    with SessionContext() as session:
        session.execute(FreeRooms.__table__.insert(), [dict({
            room: int(table.table[table.time[t], table.rooms[room]]) for room in table.rooms.keys()
        }, **{'day_period': t}) for t in sorted(table.time.keys())])
